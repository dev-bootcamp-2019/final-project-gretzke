...
contract Marketplace {
    ...
    function addAdmin(address _newAdmin) external onlyOwner stopInEmergency {
    ...
    function addStoreOwner(address _newStoreOwner) external onlyAdmin stopInEmergency {
    ...
    function removeStoreOwner(address _storeOwner) external onlyAdmin stopInEmergency {
    ...
    function addStore(string memory _name, string memory _description) public onlyStoreOwner stopInEmergency returns(bytes32 storeID) {
    ...
    function removeStore(bytes32 _storeID) public onlyStoreOwner stopInEmergency returns(bool) {
    ...
    function addItem(
        ...
        uint256 _stock
    ) public onlyStoreOwner stopInEmergency returns(bytes32 itemID) {
    ...
    /// @return true on success, false on failure
    function removeItem(bytes32 _storeID, bytes32 _itemID) public onlyStoreOwner stopInEmergency returns(bool) {
    ...
    function purchase(address _storeOwner, bytes32 _storeID, bytes32 _itemID) public payable stopInEmergency returns (bool) {
    ...
    /// @return true on success, false on failure
    function restock(bytes32 _storeID, bytes32 _itemID, uint256 _amount) public onlyStoreOwner stopInEmergency returns(bool) {
        // storage pointer to item
        Item storage item = stores[msg.sender][_storeID].items[_itemID];
        // ensure that parent store is active
        require(stores[msg.sender][_storeID].active, "store is not active");
    ...
    /// @return true on success, false on failure
    function changePrice(bytes32 _storeID, bytes32 _itemID, uint256 _price) public onlyStoreOwner stopInEmergency returns(bool) {
        // storage pointer to item
        Item storage item = stores[msg.sender][_storeID].items[_itemID];
        // ensure that parent store is active
        require(stores[msg.sender][_storeID].active, "store is not active");
    ...
    function setFeaturedStoreOwner(uint256 _index, address _storeOwner) public onlyOwner stopInEmergency {
        featuredStoreOwners[_index] = _storeOwner;
    }

    /// @notice flips the emergency switch, only callable by owner
    function flipEmergencySwitch() public onlyOwner {
